...
function fetchDataFromEl(el) {
  ...
  result['geometry'] = {height:geo.height, radius:geo.radius};
...
class InfoTypeCylinder extends Component {
  constructor(props) {
    super(props);
    this.state = {
      el: props.el
  ...
  render() {
    ...
    return (
      <div>
        <div>
          <div>Cylinder <span onClick={this.deleteObject} className="el-remove-btn">Delete</span></div>
          <div>
            position: 
          ...
          <div>
          	radiusTop
          	<input className="textInput" value={data.geometry.radius} data-value="geometry.radius" onChange={this.changeObjectField} />
          </div>
          <div>
            rotation:
...
export default InfoTypeCylinder;