...
function fetchDataFromEl(el) {
  ...
  result['material'] = {};
  let side = el.getAttribute('material').side;
  result['material']['side'] = side;
  let transparent = el.getAttribute('material').transparent;
  result['material']['transparent'] = transparent;
...
class InfoTypeImage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      el: props.el
    ...
    this.changeObjectCheck = this.changeObjectCheck.bind(this);
  ...
  changeObjectField(event) {
    ...
    // console.log(event.target.getAttribute('checked'), event.target.checked);
  ...
  changeObjectCheck(event) {
    let field = event.target.getAttribute('data-value').split('.');
    // console.log(event.target.getAttribute('checked'), event.target.checked);
    let tmp = this.state.data[field[0]];
    tmp[field[1]] = event.target.checked;
    this.state.el.setAttribute(field[0], tmp);
    this.state.data[field[0]][field[1]] = event.target.checked;
    this.setState({
      data: this.state.data
    })
    Events.emit('objectchanged',this.state.el.object3D);
    ...
    return (
      <div>
        <div>
          ...
          <div>
            transparent (not necessary):
            <input type="checkbox" checked={data.material.transparent} data-value="material.transparent" onChange={this.changeObjectCheck} />
          </div>
          <div>
            opacity:
            <input className="textInput" value={data.material.opacity} data-value="material.opacity" onChange={this.changeObjectField} />
          </div>
          <div>
