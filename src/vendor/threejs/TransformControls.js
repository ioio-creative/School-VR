			...
			if ( pointer.button === 0 || pointer.button === undefined ) {
				...
				if ( intersect ) {
					...
					if ( planeIntersect ) {
						...
						if (scope.object.initscale === undefined) {
							scope.object.initscale = new THREE.Vector3(scope.object.scale.x, scope.object.scale.y, scope.object.scale.z);
						}
			...
			if ( _mode === "translate" ) {
				...
				if ( scope.space === "local" || _mode === "scale" ) {
					...
					if ( scope.axis === "XYZ" ) {
						...
						// console.log('scope.axis === "XYZ"', scale);

						if (scope.scaleSnap != null) {
							scope.object.scale.x = scope.object.initscale.x * (Math.round( scale * oldScale.x / scope.scaleSnap ) * scope.scaleSnap);
							scope.object.scale.y = scope.object.initscale.y * (Math.round( scale * oldScale.y / scope.scaleSnap ) * scope.scaleSnap);
							scope.object.scale.z = scope.object.initscale.z * (Math.round( scale * oldScale.z / scope.scaleSnap ) * scope.scaleSnap);
						} else {
						...
						point.applyMatrix4( tempMatrix.getInverse( worldRotationMatrix ) );
						// console.log('scope.axis !== "XYZ"', point);

						if (scope.scaleSnap != null) {
							if ( scope.axis === "X" ) {
								var realX = oldScale.x + point.x;
								scope.object.scale.x = scope.object.initscale.x *  (Math.round( realX / scope.scaleSnap ) * scope.scaleSnap);
							}
							if ( scope.axis === "Y" ) {
								var realY = oldScale.y + point.y;
								scope.object.scale.y = scope.object.initscale.y *  (Math.round( realY / scope.scaleSnap ) * scope.scaleSnap);
							}
							if ( scope.axis === "Z" ) {
								var realZ = oldScale.z + point.z;
								scope.object.scale.z = scope.object.initscale.z *  (Math.round( realZ / scope.scaleSnap ) * scope.scaleSnap);
							}
						} else {
					...
					// prevent scale = 0 and get error
					scope.object.scale.x = (scope.object.scale.x == 0 ? 0.01: scope.object.scale.x);
					scope.object.scale.y = (scope.object.scale.y == 0 ? 0.01: scope.object.scale.y);
					scope.object.scale.z = (scope.object.scale.z == 0 ? 0.01: scope.object.scale.z);
				...
				if ( scope.axis === "E" ) {
					...
					// added rotation snap on E axis, the outermost yellow one
					if ( scope.rotationSnap !== null ) {
						quaternionE.setFromAxisAngle( eye, Math.round( ( rotation.z - offsetRotation.z ) / scope.rotationSnap ) * scope.rotationSnap );
					} else {
					...
					// add to enable the rotation of the a-frame camera
					if (scope.object.el && scope.object.el.components['look-controls']) {
						scope.object.el.components['look-controls'].yawObject.rotation.copy(scope.object.rotation);
						scope.object.el.components['look-controls'].pitchObject.rotation.copy(scope.object.rotation);
					}
				}
